...
def add_callback():
    ...
    if 'df_quydoi_l' not in st.session_state or st.session_state.quydoi_gioday['df_quydoi_l'].empty or 'Stt_Mon' not in st.session_state.quydoi_gioday['df_quydoi_l'].columns:
        ...
        st.session_state.quydoi_gioday['df_quydoi_l'] = pd.concat([st.session_state.quydoi_gioday['df_quydoi_l'], new_row_df], ignore_index=True, sort=False)
...
def taonhaplop_mon_par(i1,chuangv_f):
    ...
    # --- CHỌN LỚP GHÉP (nhomlop == 1) ---
        ...
        if lop_ghep_zero:
            ...
            part_dslopghep = os.path.join(data_parquet_dir, 'df_lopgheptach_gv.parquet')  # Đường dẫn file lớp ghép cụ thể cho GV
            st.write(f':orange[Thông tin lớp ghép (sĩ số theo tháng)]')
            # Gọi hàm xử lý và lưu lớp ghép
            #lop_ghep, lop_ghep_t, lop_ghep_m
            lopghep_xep, lop_ghep_m, lopghep_chinh, lopghep_machinh = fun_lopghep.transform_and_sort_lopghep(tenlop_chon)
    ...
    # Các trường hợp nhomlop == 2, 3 (Tách lớp, Ghép + Tách lớp)
        ...
        if not df_lop_g.empty and 'Lớp' in df_lop_g.columns:
            tenlop_data_original = str(st.session_state.quydoi_gioday['df_quydoi_l']['Chọn lớp'].iloc[i1])  # Đảm bảo là string
            if tenlop_data_original in dslop_options:
                index_lop = dslop_options.index(tenlop_data_original)
            else:
    ...
    if kt_mon_tontai:
        ...
        if (not quydoi_data_old.empty) and laymau_quydoi == False:
            ...
            quydoi_tiet = quydoi_data_old['Tiết'] # Giữ nguyên kiểu dữ liệu gốc
            ...
            # Nếu không, gán giá trị mặc định (1 và 12)
        ...
        with col1:
            tuandentuan = st.slider(f':blue[IV - THỜI GIAN DẠY (Tuần bắt đầu - Tuần kết thúc):]', 1, 50, (quydoi_tuan_begin, quydoi_tuna_end),
                                    key=f'Tuần{i1 + 1}')
    ...
    if kt_mon_tontai and kt_tuan_tontai:
        ...
        if tong_so_tuan_chon == arr_tiet.size and kt_lop_tontai and kt_mon_tontai:
            ...
            if chua_capnhat == False:
                ...
                df_listketqua["QĐ thừa"] = (df_listketqua["Tiết_LT"] * df_listketqua["HS_SS_LT"]) + (df_listketqua["HS_SS_TH"]*df_listketqua["Tiết_TH"])
                ...
                df_listketqua["QĐ thiếu"] = df_listketqua["HS TC/CĐ"]*((df_listketqua["Tiết_LT"] * df_listketqua["HS_SS_LT_tron"]) + (df_listketqua["HS_SS_TH_tron"]*df_listketqua["Tiết_TH"]))
                ...
                numeric_cols_to_re_clean_after_edit = [
                    "Tiết", "Sĩ số", "HS TC/CĐ","Tiết_LT","Tiết_TH", "HS_SS_LT","HS_SS_TH", "QĐ thừa", "HS thiếu", "QĐ thiếu"
                ]
                ...
                cot_lock_tudong = ['Tuần', 'Ngày', 'Sĩ số', 'Tiết','Tiết_LT','Tiết_TH', "HS TC/CĐ", "HS_SS_LT","HS_SS_TH",
                ...
                fully_styled_df_final_x = (df_final_with_total.style
                                         ...
                                         .apply(lambda r: highlight_total_row_dynamics(r, df_final_with_total), axis=1)
                                         )

                def update_total_row(df_edited, total_row_identifier_col="Ngày", total_row_identifier_value="Tổng cộng:"):
                ...
                edited_df_with_total_x = st.data_editor(
                    ...
                    column_order=["Tuần", "Ngày", "Tiết", "Sĩ số", "HS TC/CĐ", "Tiết_LT", "Tiết_TH", "HS_SS_LT", "HS_SS_TH",
                                  "QĐ thừa", "HS thiếu", "QĐ thiếu"],
                    hide_index=True,
                ...
                def compare_tiet_values_per_row(row, dataframe_for_length):
                    ...
                    if 'Tuần' in row.index and isinstance(row['Tuần'], str) and 'Tổng cộng' in row['Tuần']:
                        is_total_row = True
                    elif row.name == (len(dataframe_for_length) - 1) and not is_total_row:  # Fallback cho hàng cuối cùng
                        is_total_row = True
                ...
                with col2:
                    ...
                    st.metric(label=f"Quy đổi thiếu giờ",
                              ...
                              delta_color="normal", border=True)
                st.write(f'Mã môn: :green[{mamon}] //Tổng tiết: :green[{tongtiet_mon} (tiết)]//LT: :green[{tiet_lt}(tiết)] //TH: :green[{tiet_th}(tiết)] // KT: :green[{tiet_kt}(tiết)]')
        ...
        if not final_df_to_display.empty:
            try:
                ...
                new_column_nhomchon = [nhomlop] * len(final_df_to_display) # Tạo list chiều dài = final_df_to_display gtrị số 1
                ...
                new_column_sttmon = [i1 + 1] * len(final_df_to_display) # Tạo list chiều dài = final_df_to_display gtrị số 1
...
for i, tab_obj in enumerate(tabs[:-1]):
    with tab_obj:
        ...
        # BIẾN NÀY LÀ LIST -> DÙNG if list:
        # Thêm các dòng này vào ngay trước dòng 1386 để gỡ lỗi
        # Lấy DataFrame từ session state một cách an toàn
        malop_mamon_df = st.session_state.quydoi_gioday.get('malop_mamon', pd.DataFrame())
        if not malop_mamon_df.empty:
            st.session_state.quydoi_gioday['list_of_malop_mamon'].append(malop_mamon_df.copy())
        else:
            st.warning(f"DataFrame mã lớp/môn học rỗng sau khi xử lý môn {i + 1}, không thêm vào danh sách.")
...
with tabs[-1]: # Truy cập tab cuối cùng
    ...
    if 'list_of_df_quydoi_l' in st.session_state.quydoi_gioday and st.session_state.quydoi_gioday['list_of_df_quydoi_l']:
        summary_data_list = []
...
if st.session_state.quydoi_gioday['list_of_df_quydoi_l']:
    st.session_state.quydoi_gioday['combined_quydoi_df'] = pd.concat(st.session_state.quydoi_gioday['list_of_df_quydoi_l'], ignore_index=True)

else:
    st.info("Danh sách các DataFrame quy đổi rỗng.")

if st.session_state.quydoi_gioday['list_of_malop_mamon']:
    st.session_state.quydoi_gioday['combined_malop_mamon'] = pd.concat(st.session_state.quydoi_gioday['list_of_malop_mamon'], ignore_index=True)
