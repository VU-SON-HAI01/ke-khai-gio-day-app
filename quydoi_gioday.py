    ...
    with tab:
        ...
        if kieu_tinh_mdmh == 'LT':
            ...
            def update_tiet_lt():
                ...
                st.session_state.mon_hoc_data[i]['tiet_lt'] = tiet_lt_str
                st.info(f"Dữ liệu tiết thực hành được hiểu: {tiet_th_list} Và được đưa vào cột Tiết TH")
                st.info(f"Dữ liệu tiết lý thuyết được hiểu: {tiet_lt_list} Và được đưa vào cột Tiết LT")
                current_input['tiet'] = tiet_value
                current_input['tiet_th'] = tiet_th_str
...
with tabs[-1]:
    ...
    if st.session_state.mon_hoc_data:
        ...
        def display_summary_inputs_and_results():
            """
            Hiển thị bảng tổng hợp các input và kết quả quy đổi cho từng môn học, chia theo học kỳ.
            - Tính toán các trường tổng hợp (QĐ thừa, QĐ thiếu, Tiết, Học kỳ, ...)
            - Hiển thị bảng dữ liệu cho từng học kỳ
            """
