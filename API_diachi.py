...
if st.button("Xác nhận địa chỉ"):
	# Địa chỉ người dùng chọn từ API
	...
	# Chuyển đổi địa chỉ cũ sang mới nếu có nhập liệu
	mapping = load_mapping()
	old_province = st.session_state.get('Tên Tỉnh/Thành phố cũ', '')
	old_district = st.session_state.get('Tên Huyện cũ', '')
	old_ward = st.session_state.get('Tên Xã cũ', '')
	type_province = st.session_state.get('Loại Tỉnh/Thành phố', 'Tỉnh')
	type_district = st.session_state.get('Loại Huyện', 'Huyện')
	type_ward = st.session_state.get('Loại Xã', 'Xã')
	# Nếu có nhập địa chỉ cũ thì chuyển đổi
	if old_province or old_district or old_ward:
		new_province = convert_address(old_province, type_province, mapping)
		new_district = convert_address(old_district, type_district, mapping)
		new_ward = convert_address(old_ward, type_ward, mapping)
		st.info(f"Địa chỉ mới sau chuyển đổi: {type_province} {new_province}, {type_district} {new_district}, {type_ward} {new_ward}")
