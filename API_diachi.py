...
def page_diachi():
	...
	province_idx = st.selectbox("Tỉnh/TP (Cũ)", province_names, index=0, key="province_old") if province_names else None
	...
	district_idx = st.selectbox("Quận/Huyện (Cũ)", district_names, index=0, key="district_old") if district_names else None
	...
	ward_idx = st.selectbox("Xã/Phường (Cũ)", ward_names, index=0, key="ward_old") if ward_names else None
	ward_code = ward_codes[ward_names.index(ward_idx)] if ward_names and ward_idx else None
	street_address = st.text_input("Địa chỉ chi tiết (có thể bỏ trống)", key="street_old")
	if st.button("Xác nhận địa chỉ", key="xacnhan_old"):
