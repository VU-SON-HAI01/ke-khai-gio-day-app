...
try:
    ...
    CLIENT_EMAIL = st.secrets["gcp_service_account"]["client_email"] # Lấy email của service account
...
def get_folder_id(drive_service, folder_name, client_email):
    ...
    try:
        ...
        if folders:
            ...
            # Nếu không tìm thấy, hướng dẫn người dùng
            st.error(f"Lỗi: Không tìm thấy thư mục '{folder_name}'.")
            st.warning(
                f"Vui lòng đảm bảo rằng:\n"
                f"1. Bạn đã tạo một thư mục có tên chính xác là **'{folder_name}'** trong Google Drive.\n"
                f"2. Bạn đã chia sẻ thư mục đó với email của Service Account: **{client_email}** và cấp cho nó quyền **'Người chỉnh sửa' (Editor)**."
            )
            return None
    except Exception as e:
