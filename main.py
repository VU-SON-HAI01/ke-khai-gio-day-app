...
try:
    ...
    st.error(
        f"Lỗi: Không tìm thấy thông tin cấu hình '{e.args[0]}' trong st.secrets. Vui lòng kiểm tra file secrets.toml.")
    st.stop()
...
def get_folder_id(_drive_service, folder_name):
    ...
    try:
        ...
        if folders:
            ...
            st.error(
                f"Không tìm thấy thư mục '{folder_name}'. Vui lòng tạo và chia sẻ thư mục này với email của Service Account.")
            return None
    except Exception as e:
    ...
    if not st.session_state.initialized:
        with st.spinner("Đang xác thực và chuẩn bị môi trường làm việc..."):
            ...
            if not user_info or 'email' not in user_info:
                st.error("Không thể lấy thông tin email người dùng.")
                st.session_state.token = None
                st.stop()
            ...
            if magv:
                ...
                spreadsheet = get_or_create_spreadsheet_in_folder(gspread_client, drive_service, folder_id,
                ...
                if teacher_info:
                    ...
                    st.error(
                        f"Đã xác thực nhưng không tìm thấy thông tin chi tiết cho Mã GV: {st.session_state.magv} trong dữ liệu cục bộ.")
                    st.stop()
            else:
