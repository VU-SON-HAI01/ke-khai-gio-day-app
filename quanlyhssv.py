...
with col1:
    with st.form("form_thong_tin_chung"):
        ...
        noi_sinh_cu = st.selectbox("TỈNH (CŨ)", ["Đắk Lắk", "Khác"])
        noi_sinh_moi = st.selectbox("TỈNH (MỚI)", ["Đắk Lắk", "Khác"])
        st.markdown("QUÊ QUÁN")
        que_quan_cu = st.selectbox("TỈNH (CŨ)", ["Đắk Lắk", "Khác"])
        que_quan_moi = st.selectbox("TỈNH (MỚI)", ["Đắk Lắk", "Khác"])
        st.divider()
